...
const order = (over = {}) => ({ status: 'Order Complete', total_usd: 100, total_crc: 45000, ...over });
...
test('exclude beats a status that would otherwise count', () => {
  const test1 = order({ status: 'Order Complete', stats_override: 'exclude' });
...
test('an unknown or empty override is ignored rather than trusted', () => {
  ...
  assert.equal(orderCountsAsSale(order({ status: 'Order Complete', stats_override: 'maybe' })), true);
});

test('the override is read case-insensitively', () => {
  assert.equal(orderCountsAsSale(order({ status: 'Order Complete', stats_override: 'EXCLUDE' })), false);
...
test('a row from a database without the column behaves exactly as before', () => {
  ...
  const legacy = { status: 'Order Complete', total_usd: 100, total_crc: 45000 };
...
test('excluded orders drop out of a total, included ones are added', () => {
  const rows = [
    order({ status: 'Order Complete', total_usd: 100 }),
    order({ status: 'Order Complete', total_usd: 250, stats_override: STATS_OVERRIDE_EXCLUDE }),
...
test('withoutExcludedOrders drops only the excluded ones', () => {
  const keep1 = order({ status: 'Order Complete' });
  const drop = order({ status: 'Order Complete', stats_override: 'exclude' });
...
test('the drill-down can say why each order does or does not count', () => {
  assert.deepEqual(orderRevenueBasis(order({ status: 'Order Complete' })),
    { counts: true, override: null, reason: 'Counts because status is Order Complete' });
  assert.deepEqual(orderRevenueBasis(order({ status: 'Pending' })),
    { counts: false, override: null, reason: 'Status Pending is not a sale' });
  assert.deepEqual(orderRevenueBasis(order({ status: 'Order Complete', stats_override: 'exclude' })),
    { counts: false, override: 'exclude', reason: 'Excluded by hand (status is Order Complete)' });
