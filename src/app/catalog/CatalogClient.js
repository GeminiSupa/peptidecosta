...
// (shared helpers live in @/lib/catalogProducts)

// Costa Rica Provinces and Cantons dataset — loaded when checkout opens
...
export default function CatalogClient({
  ...
  useEffect(() => {
    ...
    const fetchMetadata = async () => {
      try {
        ...
        // Ignore geolocation failures (ad blockers, offline)
      }
    };
    if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
      window.requestIdleCallback(() => fetchMetadata(), { timeout: 5000 });
    } else {
    ...
    if (initialProducts?.length) {
      ...
      if (isSupabaseConfigured && supabase) {
        supabase
          ...
          .select('product,rating,review_text,reviewer_name,created_at,status')
          .eq('status', 'Approved')
          .then(({ data }) => {
  ...
  // Load PayPal JS SDK only when user selects PayPal (saves ~100KB+ on initial load)
  ...
  useEffect(() => {
    if (paymentMethod === 'paypal') loadPayPalSdk();
  }, [paymentMethod, loadPayPalSdk]);

  useEffect(() => {
    const timer = setTimeout(() => setShowPromoBanner(true), 1200);
    return () => clearTimeout(timer);
  }, []);

  useEffect(() => {
    if (!isCartOpen || costaricaData) return;
    import('@/lib/costarica.json').then((mod) => {
      setCostaricaData(mod.default || mod);
    });
  }, [isCartOpen, costaricaData]);
  ...
  const loadCatalogData = async () => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        const { data, error } = await supabase
          .from('products')
          .select('product,category,price_usd,price_crc,original_price_usd,original_price_crc,discount,sale_start_time,sale_end_time,status,inventory_count,low_stock_threshold,coa,image_url,description_en,description_es,emoji,priority')
          .order('priority', { ascending: true });

        if (!error && data?.length > 0) {
          loadedProducts = data.map(mapDbProduct);
          setIsDbBacked(true);
        } else {
          setIsDbBacked(false);
        }
          ...
          Papa.parse(csvText, {
            ...
            complete: (results) => {
              ...
              const headerIndex = lines.findIndex((l) =>
                l.some((cell) => cell && cell.toLowerCase().includes('product'))
              );
  ...
  const handleThemeToggle = (selectedTheme) => {
    startTransition(() => setTheme(selectedTheme));
  ...
  const scheduleFlyingAnimation = useCallback((productData, startRect, endRect) => {
    ...
    startTransition(() => {
      setFlyingItems((prev) => [
        ...
        {
          id,
          imageSrc: productData.imageUrl || '/catalog/peptides_2.png',
          startX,
          startY,
          endX,
          endY,
  ...
  // Cart operations — functional updates keep taps fast (better INP)
  const addToCart = useCallback((productObj, { openDrawer = true } = {}) => {
    let blocked = false;
    setCart((prev) => {
      const existing = prev.find((item) => item.product === productObj.product);
      ...
      if (existing) {
        return prev.map((item) =>
          item.product === productObj.product ? { ...item, qty: newQty } : item
        );
      }
      return [...prev, { ...productObj, qty: 1 }];
    });

    if (blocked) {
      alert(
        lang === 'en'
          ? `Only ${productObj.inventoryCount} units available in stock.`
          : `Solo ${productObj.inventoryCount} unidades disponibles en inventario.`
      );
      return;
    }
  ...
  const addToCartWithAnimation = useCallback((e, productData) => {
    e.stopPropagation();
    addToCart(productData, { openDrawer: false });
    setAddedProductId(productData.product);
    setTimeout(() => setAddedProductId(null), 600);

    const button = e.currentTarget;
    requestAnimationFrame(() => {
      let startRect = button.getBoundingClientRect();
      ...
      if (cartBtn) endRect = cartBtn.getBoundingClientRect();

      scheduleFlyingAnimation(productData, startRect, endRect);
  ...
  }, [addToCart, scheduleFlyingAnimation]);
  ...
  const updateCartQty = useCallback((productName, change) => {
    setCart((prev) =>
      prev
        .map((item) => {
          ...
          if (change > 0 && item.inventoryCount !== null && newQty > item.inventoryCount) {
            alert(
              lang === 'en'
                ? `Only ${item.inventoryCount} units available in stock.`
                : `Solo ${item.inventoryCount} unidades disponibles en inventario.`
            );
            return item;
          }
  ...
  // Categories + filtering (memoized + deferred search for INP)
  const categoriesList = useMemo(() => {
    const sortedCategories = Array.from(new Set(products.map((p) => p.category))).filter(Boolean).sort();
    ...
    <div id="app" className="catalog-page-shell min-h-screen" suppressHydrationWarning>
        ...
        <div className="search-bar container">
          ...
          <div className={`filter-panel ${showFilters ? 'active' : ''}`}>
            <div className="filter-group">
              ...
              <select value={priceFilter} onChange={(e) => startTransition(() => setPriceFilter(e.target.value))}>
            ...
            <div className="filter-group">
              ...
              <select value={sortOrder} onChange={(e) => startTransition(() => setSortOrder(e.target.value))}>
            ...
            <div className="filter-group toggle-group">
              ...
              <input 
                type="checkbox" 
                checked={inStockOnly} 
                onChange={(e) => startTransition(() => setInStockOnly(e.target.checked))}
              />
            </div>
          </div>
        </div>
      ...
      <main className="main container" style={{ position: 'relative', minHeight: '60vh' }}>
        ...
        {!gateLoading && (
          ...
          <div className={`product-grid ${viewMode}-view${isSearchStale ? ' product-grid--pending' : ''}`}>
