...
export async function POST(request) {
  ...
  try {
    const { campaign_id, is_test_batch, send_winner, winner_variant, include_leads, target_tags } = await request.json();
    ...
    const result = await deliverCampaign(campaign_id, {
      ...
      audience: include_leads === undefined && target_tags === undefined
        ? null
        : { includeLeads: include_leads, targetTags: target_tags },
    });
