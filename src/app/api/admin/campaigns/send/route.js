...
export async function POST(request) {
  ...
  try {
    const { campaign_id, is_test_batch, send_winner, winner_variant, audience_scope, include_leads, target_tags, behavior_filter } = await request.json();
    ...
    const result = await deliverCampaign(campaign_id, {
      ...
      audience: audience_scope === undefined && include_leads === undefined && target_tags === undefined && behavior_filter === undefined
        ? null
        : { scope: audience_scope, includeLeads: include_leads, targetTags: target_tags, behaviorFilter: behavior_filter },
    });
