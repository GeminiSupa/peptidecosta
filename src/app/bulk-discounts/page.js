...
export default function BulkDiscountsPage() {
  ...
  const heroTitle = (en ? campaign.titleEn : campaign.titleEs) || (variant === 'b'
    ? (en ? 'Build your own bulk wholesale peptide order' : 'Arma tu propio pedido mayorista de péptidos')
    : (en ? `Get ${campaign.discountPct}% off when you build a ${campaign.minUnits}-vial order` : `Obtén ${campaign.discountPct}% de descuento al armar un pedido de ${campaign.minUnits} viales`));
